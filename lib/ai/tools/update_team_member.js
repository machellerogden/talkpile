...
export default {
    async impl(session, agent, { name, designation, description, instructions, tools, temperature, frequency_penalty, presence_penalty }) {
        ...
        const targetAgent = agentsConfig[designation];
...
${printTeamRoster(session, agent)}`;
        ...
        if (name) targetAgent.name = name;
        if (description) targetAgent.description = description;
        if (instructions) targetAgent.instructions = instructions;
        if (temperature != null) targetAgent.temperature = temperature;
        if (frequency_penalty != null) targetAgent.frequency_penalty = frequency_penalty;
        if (presence_penalty != null) targetAgent.presence_penalty = presence_penalty;

        console.log('********************************************************************************');
        console.log('Updating team member:', targetAgent);
        console.log('********************************************************************************');
        ...
        return `Updated team member: ${designation}
...
${printTeamRoster(session, agent)}`;
    ...
    parameters: {
        ...
        properties: {
            designation: {
                type: 'string',
                description: 'The official designation by which the team member will be referred. Specifies which agent to update.'
            },
            name: {
                type: 'string',
