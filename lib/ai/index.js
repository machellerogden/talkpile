...
export async function* Delegate(session, options) {
    ...
    async function* applyTool(role, toolId, toolName, toolArgs) {
        ...
        try {
            ...
            const { loc, action, orientation, operations, label, desc, message, assignee } = toolArgs;
            ...
            if (toolName === 'delegate') {
                if (assignee in session.delegates) {
                    ...
                    toolResponse = yield* await session.delegates[assignee](message, agent.name, role);
