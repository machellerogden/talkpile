...
import { truncateChatCompletion } from './request.js';
...
export async function* Agent(session, options) {
    ...
    repl: while (true) {
        ...
        const completionRequest = truncateChatCompletion(
            {
                model,
                temperature,
                frequency_penalty,
        ...
        if (stream.error) {
            yield fx(
                'send-error',
                `error with chat completion`,
                completionRequest,
                stream.error.stack
            );
            yield fx('send-chunk', EOL + 'Error with chat completion. Please try again.' + EOL);
            prompt = true;
            continue repl;
        }
    ...
    if (exitInterview) {
        ...
        const exitInterviewRequest = truncateChatCompletion(
            {
                model,
                temperature,
                frequency_penalty,
        ...
        yield fx('send-debug-log', printPrefix('delegate', COLOR.info) + ' ' + content);

        const exitInterviewResponse = yield fx('request-chat-completion', exitInterviewRequest);
        const summary =
            exitInterviewResponse?.choices?.[0]?.message.content ?? agent.designation + ' done';

        yield fx('send-debug-log', printKV('summary response', summary));
    ...
    async function* applyTool(role, toolId, toolName, toolArgs) {
        ...
        try {
            ...
            if (toolName === 'message_team_member') {
                ...
                try {
                    toolResponse = yield fx('tool-call', agent, toolName, toolArgs);
                } catch (toolError) {
                    toolResponse =
                        'Error from tool ' +
                        toolName +
            ...
            const toolCallResponseMessage = {
                ...
                role: 'tool',
                name: toolName,
                content: toolResponseToString(toolResponse)
            };
...
function toolResponseToString(toolResponse) {
    if (toolResponse instanceof Error) {
        return toolResponse.stack;
    }
    return [
        'string',
        'number',
        'boolean'
    ].includes(typeof toolResponse)
        ? toolResponse
