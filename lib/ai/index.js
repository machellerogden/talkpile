...
export async function* Delegate(session, options) {
    ...
    if (exitInterview) {
        ...
        return `Task completed. Here is a summary from team member ${agent.designation}: \n\n${summary}`;
    }
    ...
    async function* applyTool(role, toolId, toolName, toolArgs) {
        ...
        try {
            ...
            if (toolName === 'delegate') {
                if (assignee in session.delegates) {
                    ...
                    throw new Error(`delegate ${assignee} not found in session.delegates`);
                }
...
export async function getTools(session, agent) {
    ...
    for (const toolName of (agent.tools ?? [])) {
        tools[toolName] = builtinTools[toolName] ?? (() => `No function found for tool: ${toolName}`);
    }
...
export async function packageDelegates(session) {
    ...
    for (const agent of Object.values(session.agents)) {
        ...
        try {
            // Delegate, I hereby appoint you as my delegate to be called upon when
            ...
            delegates[agent.designation] = async (task, from, role) => {
                ...
                return Delegate(session, options);
            }
        } catch (error) {
