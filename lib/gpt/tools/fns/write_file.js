...
export default async function write_file(session, { file_path, cwd = process.cwd(), data, encoding = 'utf8', mode = '666', append = false }) {
    let response;
    const confirm = await (new enquirer.Confirm({
        name: 'confirm',
        message: `Write file: ${file_path} (cwd: ${cwd}, encoding: ${encoding}, mode: ${mode}, append: ${append})?`
    })).run();
    if (!confirm) return 'File not written. Reason: User declined.';
    try {
        const options = {};
