...
const loadKit = ({ messages, tools, fns, prelude }) => {
    messages = messages ?? [];
    tools = tools ?? [];
    prelude = prelude ?? [];
    fns = fns ?? {};
    return { messages, tools, fns, prelude };
}

export async function* GPT(session = {}, kit = {}) {
    const { openai, name, debug } = session;
    const { messages, tools, fns, prelude } = loadKit(kit);
        ...
        try {
            ...
            if (fnName in fns) {
                if (typeof fns[fnName] === 'function') {
                    toolResponse = await fns[fnName](session, fnArgs);
                    if (debug) console.log('toolResponse', inspect(toolResponse));
                } else {
                    toolResponse = fns[fnName];
                }
            } else {
            ...
            yield fx('send-error', printPrefix(role, COLOR.assistant) + 'used ' + fnName + ' tool but there was an error:' + e.stack);
        }
    }
