...
const loadKit = ({ messages, tools, fns, fxs, prelude }) => {
    messages = messages ?? [];
    tools = tools ?? [];
    prelude = prelude ?? [];
    fns = fns ?? {};
    fxs = fxs ?? {};
    return { messages, tools, fns, fxs, prelude };
};

export async function* GPT(session = {}, kit = {}) {
    const { name, debug } = session;
    const { messages, tools, fns, fxs, prelude } = loadKit(kit);
    ...
    repl: while (true) {
        ...
        const stream = yield fx('create-chat-stream', completionRequest);
