...
const defaultIdentity = `As an AI agent embedded in a command-line interface (CLI) tool, you serve as a dynamic copilot assisting users with a wide range requests.`;
...
export async function packageDelegates(session) {
    ...
    for (const [ kitName, kitConfig ] of Object.entries(config.kits)) {
        ...
        try {
            ...
            kitConfig.identity = kitConfig.identity ?? defaultIdentity;
...
export async function packageFn(fn, session, config) {
    fn = typeof fn === 'function'
        ? await packageFn(await fn(session, config), session, config)
        : {
            type: 'function',
            'function': {
                name: fn.name,
...
export async function packageFns(fns, session, config) {
    return Object.values(fns).reduce(async (acc, fn) => {
        acc = await acc;
        acc.push(await packageFn(fn, session, config));
        return acc;
    }, []);
}
