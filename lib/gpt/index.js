...
import { core, builtin } from './tools/index.js';
...
export async function packageDelegates(session) {
    ...
    for (const [ kitName, kitConfig ] of Object.entries(config.kits)) {
        ...
        try {
            ...
            kitConfig.import = kitConfig.import ?? `./kits/team.js`;
            ...
            for (const fn of (kitConfig.fns ?? [])) {
                kit.fns[fn] = builtin[fn] ?? (() => `No function found for ${fn}`);
            }
