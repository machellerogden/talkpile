...
export async function* GPT(session = {}, kit = {}, message = {}) {
    ...
    const contextPrefix = session.config.prompt() + session.prefixes.map(prefix => printPrefix(prefix, COLOR.mode)).join('');

    const user = {};

    if (name) user.name = name;

    const delegated = !!message?.content?.length;

    let prompt = !delegated;
    ...
    if (delegated) {
        ...
        const prelude = await getPrelude(session);
        const tools = await getTools(session);
        const content =  'Please provide a concise summary explaining the outcome of this conversation. This will be used to debrief the team about your session with the user.';
        ...
        summary = outcomeSummaryResponse?.choices?.[0]?.message.content;
        yield fx('send-log', printKV('summary response', summary));

    }

    return summary ?? kit.command + ' done';
    ...
    async function* applyFn(role, fnId, fnName, fnArgs) {
        try {
            ...
            if (fnName === 'delegate' && assignee in session.delegates) {
                ...
                toolResponse = yield* await session.delegates[assignee](task);
                ...
                try {
                    ...
                    toolResponse = 'Error from tool ' + fnName + ' with ' + fnArgs + ' - ERROR - ' + toolError.stack;
                }
