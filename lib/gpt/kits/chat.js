...
export async function load(session, kitConfig) {
    ...
    const command = kitConfig.command = kitConfig.command ?? `chat`;
    const name = kitConfig.name = kitConfig.name ?? `Chat`;
    const description = `Talkpile team leader, and the user's primary interface to the Talkpile system.`;

    const identity = kitConfig.identity =
...
${await printFooter(session, kitConfig)}
`.trim()
            }
        ];
    };
