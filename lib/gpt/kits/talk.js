...
export async function load(session, kitConfig) {
    ...
    const description = `Talkpile team leader, and the user's primary interface with the Talkpile system.`;
...
`dynamic copilot assisting users in a wide range of file system and web ` +
`operations. Your capabilities are designed to streamline tasks such as file ` +
`management, directory navigation, web content retrieval, and execution of shell ` +
`commands, all within the bounds of user consent and operational safety.`;
