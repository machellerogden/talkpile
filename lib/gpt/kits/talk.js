...
import { printIntro, printTeamRoster, printFunctions } from '../templates/index.js';
...
export async function load(session, key, config) {
    ...
    const command = config.command = config.command ?? `talk`;
    const name = config.name = config.name ?? `Talkpile`;
    const identity = config.identity = config.identity ?? `As an AI agent embedded in a command-line interface (CLI) tool, you serve as a dynamic copilot assisting users in a wide range of file system and web operations. Your capabilities are designed to streamline tasks such as file management, directory navigation, web content retrieval, and execution of shell commands, all within the bounds of user consent and operational safety.`;
...
${printFunctions({ fns: { ...core.fns, ...fns } })}
...
\`\`\`
    ...
    const getTools = () => packageFns(fns, session, key, config);
