...
export const packageFn = async (fn, session, key, config) => {
    fn = typeof fn === 'function'
        ? await packageFn(await fn(session, key, config), session, key, config)
        : {
            type: 'function',
            function: {
                name: fn.name,
...
export const packageFns = (fns, session, key, config) =>
    Object.values(fns).reduce(async (acc, fn) => {
        acc = await acc;
        acc.push(await packageFn(fn, session, key, config));
        return acc;
    }, []);
...
export function getTeamRoster(session, requester) {
