...
export const printIntro = (session, kit) => `
Your name is ${kit.name}. The user has requested you using the command \`${kit.command}\`.
...
${team.map(({ name, command, description }) => `- **${command}**: Also known as ${name}. ${description}`).join('\n')}`;

    return teamText;
}

export const printTeamRoster = (session, kit) => {

    const team = Object.values(session.kits)
        .filter(({ command:c }) => c != kit.command)
...
export const printFunctions = async (session, kit) => {
   const tools = await packageFns(session, kit, { ...kit.fns, ...core.fns });
   return `
**Functions Overview:**
...
export const printFooter = (session, kit) => `
**Important Context:**
...
export async function printHeader(session, kit) {
   return `
${await printIntro(session, kit)}
${await printTeamRoster(session, kit)}
${await printFunctions(session, kit)}
`.trim();
}
