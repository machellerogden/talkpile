...
export async function getConfig(env, argv, username) {
    env = env ?? process.env;
    ...
    const { input, flags, unnormalizedFlags } = meow(HELP_TEXT, {
        ...
        flags: {
            username: {
                type: 'string',
                default: preferences.get('username', username)
            },
            debug: {
                type: 'boolean',
                default: false
            ...
            openaiApiKey: {
                type: 'string',
                default: preferences.get('openaiApiKey', env.OPENAI_API_KEY)
            }
        }
    });
    ...
    // OpenAI API has restrictions on name that gets sent with user messages.
    config.name = config.username.replace(/[^a-zA-Z0-9_-]/g, '_');
