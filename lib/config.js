...
async function loadConfig(env, argv, cwd, configFilename, platform) {

    const sources = await findConfigSources(env, argv, cwd, configFilename, platform);
...
export async function getClientConfig(env, argv, cwd, platform) {
    ...
    const { sources:configSources, configs, config:baseConfig } = await loadConfig(env, argv, cwd, configFilename, platform);
