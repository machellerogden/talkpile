...
export const CONFIG_FILENAME = '.talkpile.yml';
...
export function getUserConfigPath(filename, env, platform) {
...
export function getSystemConfigPath(filename, platform) {
...
export async function loadConfig(env, argv, cwd, CONFIG_FILENAME, platform) {

    const sources = await findConfigSources(env, argv, cwd, CONFIG_FILENAME, platform);
    const configs = sources.map(source => loadYamlFileSync(source));
    let config = configs.reduce((acc, config) => merge(acc, config), {});

    return { sources, configs, config };
}
...
export async function getClientConfig(env, argv, cwd, platform) {
    ...
    const { sources:configSources, configs, config:baseConfig } = await loadConfig(env, argv, cwd, CONFIG_FILENAME, platform);
...
export async function getServiceConfig(env, argv, cwd, platform) {
...
NAME
     ${c.bold('talkpile-service')} - host piles of bots on your machine

SYNOPSIS
     talkpile-service

DESCRIPTION
     The talkpile-service command hosts talkpile on your machine.
